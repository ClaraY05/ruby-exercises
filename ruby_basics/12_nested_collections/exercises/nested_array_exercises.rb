def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  # change with it
  return Array.new(number_of_rows){Array.new(seats_per_row,nil)}
end
...
def count_empty_seats(chart)
  ...
  empty_count = 0
  chart.each{|chart_row| empty_count += chart_row.count(nil)}
  empty_count
end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.each do|c_hash|
    if(c_hash.dig(:is_my_favorite?))
      return c_hash
    end
  end
  return nil
